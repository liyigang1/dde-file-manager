...
void LocalDirIterator::cacheBlockIOAttribute()
{
    ...
    d->hideFileList = SortFileInfoUtils::loadHideFileList(this->url());
...
// This method only collects lightweight SortFileInfo for local entries.
// Sorting is handled by FileViewSorter in upper layers.
...
QList<SortInfoPointer> LocalDirIterator::sortFileInfoList()
{
    ...
    QByteArray dirPath = QFile::encodeName(d->rootPath);

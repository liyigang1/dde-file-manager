...
void FileNameSorter::sort(QStringList &fileNames, Qt::SortOrder order)
{
    ...
    // 预生成所有 sortKey
    QList<QPair<QString, QCollatorSortKey>> fileWithKeys;
    fileWithKeys.reserve(fileNames.size());
    for (const QString &name : fileNames) {
        fileWithKeys.push_back(qMakePair(name, sortKey(name)));
    }
...
void FileNameSorter::sortUrls(QList<QUrl> &urls, Qt::SortOrder order)
{
    ...
    // 预生成所有 sortKey
    QList<QPair<QUrl, QCollatorSortKey>> urlWithKeys;
    urlWithKeys.reserve(urls.size());
    for (const QUrl &url : urls) {
        urlWithKeys.push_back(qMakePair(url, sortKey(url.fileName())));
    }

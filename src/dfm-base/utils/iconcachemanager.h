 ...
 * - 主题变化时清空所有缓存（延迟去重执行）
 *
 * 缓存 Key 格式: "dfm:icon:{iconName}:{width}x{height}:{mode}:{state}"
 *
...
public:
     ...
     * @return 缓存的 QPixmap（未命中时从主题加载并写入缓存）
     */
    ...
    /**
     * @brief 构建缓存 Key
     * @return "dfm:icon:{iconName}:{width}x{height}:{mode}:{state}"
     */

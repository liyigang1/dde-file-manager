...
bool FileUtils::isDesktopFileSuffix(const QUrl &url)
{
    ...
    // create a fileinfo
    // local device file and is not symlink file use suffix
    if (!isLocalDevice(url) || symlinkTarget(url).isEmpty())
...
bool FileUtils::strIsPathOrNewWorkUrl(const QString &str)
{
    // 检查是路径，不立即执行搜索记录
    static const QString gvfsMatch { "(^/|^smb://*|^sftp://*|^ftp://*)" };
    QRegularExpression re { gvfsMatch };
    QRegularExpressionMatch match { re.match(str) };
    return match.hasMatch();
}

bool FileUtils::isMountNetWorkStart(const QString &str)
{
    static QSet<QString> net{"smb:/","smb://","ftp:/","ftp://","sftp:/","sftp://"};

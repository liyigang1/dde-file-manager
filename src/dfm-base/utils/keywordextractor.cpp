...
QStringList BooleanKeywordStrategy::extractKeywords(const QString &keyword) const
{
    ...
    // 按空白字符分割关键字
    QStringList keywords = keyword.split(whitespaceDelimiter, QString::SkipEmptyParts);
...
QStringList WildcardKeywordStrategy::extractFromWildcardPattern(const QString &pattern) const
{
    ...
    // 按空格分割并过滤
    QStringList segments = modifiedPattern.split(' ', QString::SkipEmptyParts);
...
QStringList KeywordExtractor::extractFromKeyword(const QString &keyword) const
{
    ...
    qCWarning(logDFMBase) << "KeywordExtractor: No strategy could handle keyword:" << keyword;
    return {};
}
...
{
    if (!strategy) {
        qCWarning(logDFMBase) << "KeywordExtractor: Attempted to register null strategy";
        return;
    }

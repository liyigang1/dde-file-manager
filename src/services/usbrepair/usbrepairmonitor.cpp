    ...
    const QStringList &interfaces)
{
    Q_UNUSED(interfaces)
    QString objPath = objectPath.path();
    m_pendingChecks.remove(objPath);
...
void UsbRepairMonitor::checkDeviceHealth(const QString &blockObjPath)
{
    ...
    if (fsType.isEmpty()) {
        ...
        QString deviceName;
        isUsbDevice(blockObjPath, &deviceName);
...
QString UsbRepairMonitor::getDeviceFile(const QString &blockObjPath)
{
    ...
    return blockIface.property("Device").toString();
}
...
void UsbRepairMonitor::checkMissingFilesystem(const QString &blockObjPath)
{
    ...
    if (blockIface.isValid()) {
        QVariantMap allInterfaces = blockIface.property("Interfaces").value<QVariantMap>();
        if (allInterfaces.contains(Defines::kUdisks2FilesystemIface)) {

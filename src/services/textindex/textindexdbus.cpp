...
void TextIndexDBusPrivate::handleConfigChanged()
{
    ...
    QSet<QString> currentExtensionsSet;
    for (const auto &su : m_currentSupportedExtensions) {
        currentExtensionsSet.insert(su);
    }
    QSet<QString> newExtensionsSet;
    for (const auto &su : newSupportedExtensions) {
        newExtensionsSet.insert(su);
    }

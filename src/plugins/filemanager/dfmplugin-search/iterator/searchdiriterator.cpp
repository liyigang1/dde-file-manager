...
void SearchDirIteratorPrivate::onMatched(const QString &id)
{
    if (taskId == id) {
        ...
        if (!results.isEmpty()) {
            ...
            hasConsumedResults.store(false, std::memory_order_release);   // 标记有新数据Add commentMore actions
        }
...
QList<QSharedPointer<SortFileInfo>> SearchDirIterator::sortFileInfoList()
{
    ...
    // 等待搜索结果或搜索完成/停止
    {
        ...
        while ((d->resultBuffer.isEmpty() || d->hasConsumedResults.load(std::memory_order_acquire))

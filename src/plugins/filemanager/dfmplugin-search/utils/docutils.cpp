...
{
    return Dtk::Core::DTextEncoding::convertTextEncoding(const_cast<QByteArray &>(input), output,
...
std::optional<QString> convertToUtf8(const QByteArray &content, const QString &fromEncoding)
{
    if (fromEncoding.toLower() == "utf-8") {
        ...
        if (convertTextEncoding(content, out, "utf-8", fromEncoding)) {
            ...
            qWarning() << "Failed to convert encoding from" << fromEncoding << "to UTF-8";
            return std::nullopt;
        }
    }
}
...
std::optional<QString> extractHtmlContent(const QString &filePath)
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "Failed to open file for HTML extraction:" << filePath;
        return std::nullopt;
    }
...
std::optional<QString> extractFileContent(const QString &filePath)
{
    ...
    try {
        ...
        qWarning() << "Exception while extracting file content:" << filePath << e.what();
        return std::nullopt;
    }

...
void AbstractWorker::getAction(AbstractJobHandler::SupportActions actions)
{
    ...
    } else if (actions.testFlag(AbstractJobHandler::SupportAction::kDeleteAction)) {
        currentAction = AbstractJobHandler::SupportAction::kDeleteAction;
    } else {
                                     ...
                                     const quint64 id, const QString &errorMsg, const bool allUsErrorMsg)
{
    ...
    fmWarning() << "Work error occurred - job type:" << jobType
                << "error type:" << error
                << "source:" << from
                << "target:" << to
                << "message:" << errorMsg;
...
void AbstractWorker::saveOperations()
{
    ...
    if (jobType == AbstractJobHandler::JobType::kCopyType
        ...
        emit requestBoardcastFiles(sourceUrl, targetUrl, completeSourceFiles);
    }
}

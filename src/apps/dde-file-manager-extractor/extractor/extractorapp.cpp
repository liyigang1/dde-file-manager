...
void ExtractorApp::run()
{
    ...
    // Connect signals
    connect(m_workerPipe.data(), &EXTRACTOR_NAMESPACE::WorkerPipe::batchReceived,
            this, [this](const QVector<QString> &filePaths) {
                processBatch(filePaths);
            });

    connect(m_workerPipe.data(), &EXTRACTOR_NAMESPACE::WorkerPipe::activityDetected,
            this, &ExtractorApp::resetIdleTimer);

    connect(m_workerPipe.data(), &EXTRACTOR_NAMESPACE::WorkerPipe::stdinClosed,
            this, []() {

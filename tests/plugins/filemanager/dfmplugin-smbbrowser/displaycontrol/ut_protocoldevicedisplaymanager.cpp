...
{
    bool isSamba = false;
    stub.set_lamda(&DeviceUtils::isSamba, [&] { __DBG_STUB_INVOKE__ return isSamba; });
...
TEST_F(UT_ProtocolDeviceDisplayManagerPrivate, IsSupportVEntry)
{
    ...
    ProtocolDeviceDisplayManager::instance()->d->showOffline = true;
    stub.set_lamda(&DeviceUtils::isSamba, [] { __DBG_STUB_INVOKE__ return false; });
    EXPECT_FALSE(ProtocolDeviceDisplayManager::instance()->d->isSupportVEntry(QUrl::fromLocalFile("/home")));

    stub.set_lamda(&DeviceUtils::isSamba, [] { __DBG_STUB_INVOKE__ return true; });
    EXPECT_FALSE(ProtocolDeviceDisplayManager::instance()->d->isSupportVEntry(QUrl::fromLocalFile("/home")));
